...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const {friendlySharedSecret, sharedSecret} = SecurityService.generateSharedSecret();
    await SecurityService.saveSharedSecret(user.id, sharedSecret);
    EmailService.sendSharedSecret(email, friendlySharedSecret);

    res.json({message: 'User registered. Please check your email for your shared secret.'});
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const passwordMatches = await UserService.comparePasswords(password, user.passwordHash);
