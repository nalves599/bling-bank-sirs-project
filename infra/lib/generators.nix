{
  ...
  */
  ...
  mkMicroVmHosts = hosts:
    lib.mapAttrs' (name: host: {
      name = "vm-${name}";
      value = host.config.microvm.declaredRunner;
    })
    (lib.filterAttrs (_: host: host.config.microvm.guest.enable) hosts);
in {
  inherit mkProfiles mkHosts mkPkgs mkOverlays mkSecrets mkLabs mkStaticConfigs mkMicroVmHosts;
}
